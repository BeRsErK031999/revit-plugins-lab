...
Встроенный PNG-runner дополнительно передаёт внутри процесса `IsoFieldLegend` с упорядоченными RGB-уровнями, числовыми границами `MinimumValue`/`MaximumValue` в `см²/м`, а также `IsoFieldLegendBoundary` с подписью сочетания диаметр/шаг и confidence. Числа принимаются только при полном распознавании всех границ и строгой монотонности шкалы; подписи — только полным упорядоченным набором без частичного результата. В JSON-схему `1.0` эти поля пока не сериализуются, поэтому внешний CLI остаётся обратно совместимым.
