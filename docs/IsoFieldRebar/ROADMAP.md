...
- Этап 17 выполнен: методичка по текущему модулю `Армирование по изополям` доступна через кнопку `?` в окне, SVG-картинки попадают в installer/artifact `Docs` payload, а установочный validator проверяет наличие IsoField guide assets.
...
- Код: при необходимости обновить installer payload или manifests, если появятся новые assets; IsoField doc assets копируются в `Docs/assets`, а окно модуля показывает встроенную методичку через кнопку `?`.
