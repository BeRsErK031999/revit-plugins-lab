...
$corePath = Join-Path $env:APPDATA "TrueBIM\2022\Core"
$sheetNumberingManifestPath = Join-Path $env:APPDATA "TrueBIM\2022\Modules\SheetNumbering\module.json"
$scheduleColumnCollapseManifestPath = Join-Path $env:APPDATA "TrueBIM\2022\Modules\ScheduleColumnCollapse\module.json"
$moduleSettingsPath = Join-Path $env:APPDATA "TrueBIM\2022\module-settings.json"
...
try {
    ...
    Invoke-Checked -Name "Revit 2022 net48 build" -Command {
        & $dotnetPath build $projectPath --configuration Release --framework net48 --nologo --verbosity:minimal
    }

    Invoke-Checked -Name "Build Revit 2022 artifacts" -Command {
    ...
    Invoke-Checked -Name "Local Revit 2022 deploy" -Command {
    ...
    if (-not (Test-Path $jsonDependencyPath)) {
        throw "Required dependency was not deployed: '$jsonDependencyPath'."
    }

    Add-Check -Name "System.Text.Json dependency" -Passed $true -Message $jsonDependencyPath
    ...
    Write-Host ""
    Write-Host "TrueBIM Revit 2022 QA preflight summary"
    Write-Host "---------------------------------------"
