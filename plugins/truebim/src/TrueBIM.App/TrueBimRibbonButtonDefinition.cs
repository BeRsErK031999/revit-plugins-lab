...
public static class TrueBimRibbon
{
    ...
    [
        ...
        new(
            ...
            TrueBimIcon.Print,
            "Открывает единое окно печати листов TrueBIM в PDF, DWG, DXF и DWF."),
        new(
            BimViewsPanelName,
        ...
        new(
            ...
            "Создаёт ведомость по листам, отмеченным галочками в окне команды.",
            "Откройте команду, снимите галочки с лишних листов и нажмите «Создать ведомость». Фильтр и источник восстановления шаблона доступны по кнопке «Настройки» в этом же окне. TrueBIM прочитает названия из первой строки шапки, объединит сегменты и заполнит новую копию шаблона."),
        new(
            ArPanelName,
        ...
        new(
            ...
            "Рассчитывает и размещает арматуру стены или плиты по четырём картам изополей.",
            "Выберите четыре карты, проверьте найденные зоны, совместите их со стеной или плитой и рассчитайте раскладку. Арматура изменится только после сравнения с моделью и отдельного подтверждения."),
        new(
            KrPanelName,
            "TrueBIM_Lintels",
            "Перемычки",
            $"{CommandNamespace}.{nameof(LintelsCommand)}",
            TrueBimIcon.Lintels,
            "Открывает модуль автоматизации оформления перемычек.",
            "Первый этап: безопасный каркас без изменения модели. Следующие этапы добавят поиск типоразмеров, сборки, боковые виды, аннотации и изображения."),
        new(
            BimViewsPanelName,
        ...
        new(
            BimCoordinationPanelName,
            "TrueBIM_ReplaceFamilies",
            "Замена\nсемейств",
            $"{CommandNamespace}.{nameof(ReplaceFamiliesCommand)}",
            TrueBimIcon.FamilyManager,
            "Заменяет выбранные экземпляры загруженным семейством другой категории с сохранением размещения.",
            "Выберите область поиска, категории, семейства и экземпляры, затем загруженный тип замены. Результат показывает заменённые экземпляры и причины пропуска."),
        new(
            BimCoordinationPanelName,
            "TrueBIM_ClashReport",
            "Отчёт\nколлизий",
        ...
        new(
            ...
            "Проверка\nпараметров",
            $"{CommandNamespace}.{nameof(ParameterAuditCommand)}",
            TrueBimIcon.Check,
            "Проверяет параметры элементов текущего проекта по требованиям из CSV или XLSX.",
            "Read-only проверка обязательности, ожидаемых и допустимых значений, формата и диапазона. Загруженные RVT-связи подключаются по запросу пользователя."),
        new(
            GeometryPanelName,
            "TrueBIM_JoinCut",
            "Соединить /\nВырезать",
