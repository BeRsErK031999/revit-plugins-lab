...
public sealed class FinishScheduleGuideWindow : TrueBimWindow
{
    ...
    private static UIElement CreateGuideBody()
    {
        ...
        Border startBanner = TrueBimUi.CreateInfoBanner(
            "Короткий маршрут: координатор один раз настраивает проект → пользователь выбирает область расчёта → выполняет предпросмотр → проверяет отчёт → формирует и открывает спецификацию. Предпросмотр модель не меняет.",
        ...
        body.Children.Add(CreateSection(
            ...
            CreateNumberedList(
                ...
                "Проверьте строку состояния и полный отчёт. Кнопка «Копировать отчёт» становится доступной после предпросмотра или формирования.",
        ...
        body.Children.Add(CreateSection(
            ...
            CreateStatusExplanation(
                ...
                "Часть значений рассчитана, но есть предупреждения геометрии, способные повлиять на ведомость. Скопируйте отчёт и проверьте указанные помещения и элементы."),
            CreateStatusExplanation(
                "Без изменений",
                TrueBimUiSeverity.Info,
