...
public sealed class FinishScheduleGuideWindow : TrueBimWindow
{
    ...
    private static UIElement CreateGuideBody()
    {
        ...
        body.Children.Add(CreateSection(
            ...
            CreateBulletedList(
                ...
                "У включённых категорий заполнен общий текстовый параметр типа с составом отделки: «• Состав», «Состав» или проектный аналог.",
        ...
        body.Children.Add(CreateSection(
            ...
            CreateNumberedList(
                ...
                "В блоке описания выберите единый текстовый параметр типа с составом отделки. При открытии плагин заменяет сохранённый источник «Имя типа» на доступный «• Состав» или «Состав». Если подходящий параметр не найден или неоднозначен, выберите источник вручную.",
        ...
        body.Children.Add(CreateSection(
            "Шаблон без пустой строки",
            CreateParagraph("Оформление перед данными задаётся один раз в проекте и переносится в каждый новый выпуск ведомости."),
            CreateNumberedList(
                "Откройте обычную спецификацию помещений. В свойствах нажмите «Вид → Изменить» и снимите «Отделять данные пустой строкой». Нажмите ОК.",
                $"Создайте из этой спецификации шаблон вида с точным именем «{FinishRoomScheduleStyleRules.AppearanceTemplateName}».",
                "В шаблоне убедитесь, что в разделе «Вид» галочка остаётся снятой. Плагин переносит только этот раздел; поля, фильтры и сортировку он формирует по текущему расчёту.",
                "После настройки снова нажмите «Сформировать». Если шаблон отсутствует, плагин покажет инструкцию до записи данных. Уже выпущенные ведомости сохраняют своё оформление.")));
        ...
        body.Children.Add(CreateSection(
            ...
            CreateNumberedList(
                ...
                "Задайте базовое название спецификации. Каждый выпуск создаёт новую версию: к занятому имени добавляется (2), (3) и далее. Предыдущие версии сохраняют данные и размещение на листах.",
                ...
                "Каждый выпуск остаётся обычной спецификацией помещений: строку можно выбрать и найти помещения в модели. Значения выпуска сохраняются отдельно и не меняются при следующем расчёте. При включённом флажке также обновится принадлежность элементов.",
                "После успешного результата нажмите «Открыть спецификацию». Главное окно закроется, а Revit откроет новый вид с зафиксированными данными расчёта.")));

        body.Children.Add(CreateSection(
            "Какие данные читаются и куда записывается результат",
            CreateDataFlowTable(),
            CreateParagraph("Площади записываются в текстовые параметры в согласованном формате ведомости. Описания и площади разных вариантов отделки идут синхронными блоками; разные отделки следуют подряд без добавочных пустых строк. Переносы длинных описаний сохраняются; площадь выравнивается по соответствующему описанию. У строки названия нет рамки, границы шапки таблицы сохраняются.")));
        ...
        body.Children.Add(CreateSection(
            ...
            CreateBulletedList(
                ...
                "Контакт с помещением определяет принадлежность отделки. Если элемент касается нескольких помещений, выбирается помещение с наибольшим контактом; список кандидатов доступен в диагностике.",
                "Стена относится к помещениям своего базового уровня, если на нём есть помещения. Соприкасающиеся участки отделки одного типа и уровня, включая оконные откосы, получают общее помещение, когда все соседние определённые участки принадлежат ему.",
                "Для стен, полов и потолков учитывается полная площадь из встроенного параметра Revit «Площадь». Она не обрезается по высоте помещения, дверным нишам или границе контакта; каждый элемент учитывается один раз.",
                "Наклонные потолки поддерживаются. Контур помещения проверяется на высоте потолка в пределах этажа, даже если потолок расположен выше заданной высоты помещения. Перекрытие также может считаться потолком при соответствующей классификации типа.",
        ...
        body.Children.Add(CreateSection(
            ...
            CreateStatusExplanation(
                "Без изменений",
                TrueBimUiSeverity.Info,
                "Совпадающие параметры не перезаписываются. Повторное формирование всё равно добавляет новую версию ведомости. В подтверждении изменения выравнивания отделены от изменений чисел и текста."),
            CreateStatusExplanation(
    ...
    private static UIElement CreateDataFlowTable()
    {
        ...
        rows.Children.Add(CreateDataFlowRow(
            "Создаётся при каждом выпуске",
            "Новая спецификация с выбором помещений в модели и зафиксированными значениями; прежние версии сохраняются.",
            isLast: true));
        return CreateTableFrame(rows);
    }
    ...
    private static UIElement CreateSafetyGrid()
    {
        ...
        UIElement write = CreateChecklistColumn(
            ...
            "сохранение прежних данных и создание новой версии спецификации.");
    ...
    private static UIElement CreateTroubleshootingTable()
    {
        ...
        rows.Children.Add(CreateTroubleshootingRow(
            "В прежней ведомости остались старые данные",
            "Это ожидаемо: прежние версии зафиксированы. Откройте новый вид кнопкой «Открыть спецификацию» и проверьте его имя в результате формирования."));
        rows.Children.Add(CreateTroubleshootingRow(
            "Получено «Без изменений»",
            "Совпадающие параметры не требуют записи; новый выпуск ведомости всё равно создаётся отдельно.",
            isLast: true));
        return CreateTableFrame(rows);
    }
