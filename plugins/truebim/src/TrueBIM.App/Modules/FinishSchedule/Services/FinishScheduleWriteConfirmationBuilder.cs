...
public sealed class FinishScheduleWriteConfirmationBuilder
{
    public FinishScheduleWriteConfirmation Build(FinishScheduleWritePreview preview)
    {
        ...
        [
            ...
            + "Строки останутся связаны с помещениями — их можно выделить в модели. "
            + "Для каждого выпуска создаётся отдельный комплект служебных параметров помещений. "
        ...
        if (preview.Schedule.LegacyScheduleIds.Count > 0)
        {
            paragraphs.Add($"Ведомостей из прежней версии плагина: {preview.Schedule.LegacyScheduleIds.Count}. "
                           + "Перед записью их данные будут сохранены в отдельных параметрах помещений; "
                           + "у текстовых архивов будет восстановлена связь с помещениями по однозначным номерам.");
        }
