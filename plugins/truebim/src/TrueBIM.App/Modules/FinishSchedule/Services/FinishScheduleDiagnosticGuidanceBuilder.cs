...
public static class FinishScheduleDiagnosticGuidanceBuilder
{
        ...
        IReadOnlyList<FinishGeometryWarning> warnings)
    {
        ...
        guidance.Add(category == FinishPreviewCategory.Ceilings
            ? "Для потолков проверьте верхнюю границу помещений, флаг «Граница помещения» и фактический контакт с помещением. Наклонные потолки с включённой границей учитываются по фактической площади контакта; сложные несвязанные элементы разделите по помещениям или отключите категорию для этого расчёта."
            : "Для полов проверьте нижнюю границу помещений, горизонтальность элементов и отсутствие изменения формы. Сложные полы разделите на простые горизонтальные участки или отключите категорию для этого расчёта.");
    }
}
