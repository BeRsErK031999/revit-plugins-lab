...
Эта методичка относится только к текущему модулю `Армирование по изополям`. Она описывает сценарий P4.1/P5.3b: комплект карт, preview, выбор плиты, трёхточечная привязка, инженерный расчёт, ручную настройку и объединение зон, отдельное сравнение с моделью и идемпотентное применение отдельных стержней. JSON и прямые стены остаются legacy-сценарием пробной проверки.
...
16. Кнопка `Применить изменения` становится доступной только для проверенного diff с реальными изменениями. Перед записью модуль повторно читает модель; если diff устарел, применение блокируется, а таблица обновляется.
17. После подтверждения diff применяется одной транзакцией; повтор той же раскладки не создаёт дубли, ручная арматура без stable id не меняется.
...
18. Для проверки защиты от устаревшего diff после нового сравнения измените или удалите один принадлежащий модулю Rebar, затем нажмите `Применить изменения`: ожидается блокировка и обновлённая таблица без записи.
19. Повторно проверьте обновлённый diff, нажмите `Применить изменения`, подтвердите операцию, проверьте комментарии `TrueBIM IsoFieldRebar`, положение относительно отверстий и выполните Revit Undo.
20. Повторите расчёт и сравнение без изменений: ожидается `без изменений`, недоступная кнопка применения, отсутствие новой транзакции и дублей.
