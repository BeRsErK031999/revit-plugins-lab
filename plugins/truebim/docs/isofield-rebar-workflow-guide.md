...
Эта методичка относится только к текущему модулю `Армирование по изополям`. Она описывает сценарий P4.1/P5.3b/P6.2: комплект карт, preview, выбор плиты, трёхточечная привязка, инженерный расчёт, ручную настройку и объединение зон, геометрический QA, сравнение с моделью, отчёт JSON/CSV и идемпотентное применение отдельных стержней. JSON и прямые стены остаются legacy-сценарием пробной проверки.
...
18. Кнопка `Экспорт отчёта` сохраняет рядом JSON и CSV schema `1.1`. В отчёт входят зоны, итоги слоёв, hashes исходников, версия обработчика, привязка, инженерные настройки, fingerprint правил, QA fingerprint, покрытие слоёв, ошибки/предупреждения и факт их принятия; экспорт доступен и при блокирующем QA.
...
20. После подтверждения diff применяется одной транзакцией; повтор той же раскладки не создаёт дубли, ручная арматура без stable id не меняется. Отчёт можно повторить после применения, чтобы зафиксировать итоговый diff.
...
- JSON/CSV-отчёт schema `1.1` содержит строки зон, итоги и покрытие слоёв, QA fingerprint, ошибки/предупреждения, факт принятия и достаточный provenance для проверки исходников и правил; экспорт не изменяет модель.
...
- P6.2 выполняет QA и экспортирует его результат, но короткий итог после создания со ссылкой на последний отчёт пока не реализован.
