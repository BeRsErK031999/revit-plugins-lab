...
Эта методичка относится только к текущему модулю `Армирование по изополям`. Она описывает сценарий P4.1/P5.3b/P6.3: комплект карт, preview, выбор плиты, трёхточечная привязка, инженерный расчёт, ручную настройку и объединение зон, геометрический QA, сравнение с моделью, отчёт JSON/CSV и идемпотентное применение отдельных стержней. JSON и прямые стены остаются legacy-сценарием пробной проверки.
...
5. Пользователь выбирает host. Preflight относит горизонтальную плиту к engineering-профилю, прямую базовую стену — к legacy-пробе. Криволинейные, составные и витражные стены, а также плиты без распознанной горизонтальной верхней грани остаются видимыми, но расчёт и запись блокируются с точной причиной.
...
- P7.1 выполняет host-preflight до расчёта и повторяет его перед записью. Это диагностическая защита, а не поддержка сложных стен или наклонных плит.
