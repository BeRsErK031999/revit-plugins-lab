...
```text
%APPDATA%\TrueBIM\<RevitVersion>\Modules\*\module.json
```

The launcher loads manifests for the active Revit version and only includes modules whose ids map to known runtime implementations. Invalid manifests and unknown module ids are logged and skipped without breaking the launcher.
...
```text
%APPDATA%\TrueBIM\<RevitVersion>\module-settings.json
```
