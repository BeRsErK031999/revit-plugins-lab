...
## UX и встроенная методичка

В шапке окна находится общая кнопка `Методичка`. Она открывает пошаговый маршрут по
вкладкам `Проект`, `Семейства` и `Отчёт`, объясняет границы автоматизации, правила
безопасного удаления и состав данных для обращения в поддержку.

Действия сгруппированы по назначению:

- копирование идентификаторов отделено от проверок использования;
- проектная проверка и поиск в семействах занимают отдельные полноширинные строки;
- удаление вынесено в визуально изолированную опасную зону;
- вкладка семейств организована как маршрут `1. Источник → 2. Параметр → 3. Анализ`;
- рядом с пятью неоднозначными сценариями размещена кнопка контекстной справки:
  проектная проверка, поиск в семействах проекта, каталог GUID из RFA, глубокий анализ
  семейств и безопасное удаление.

Контекстная справка открывается без запуска соответствующей операции. Для действий и
полей заданы точные tooltips; у недоступной кнопки tooltip объясняет, какое условие ещё
не выполнено. Это позволяет понять следующий шаг без пробных нажатий и без изменения
модели.
